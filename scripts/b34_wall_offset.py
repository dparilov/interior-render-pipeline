...
"""
B34 — Camera offset by wall thickness (productized)

Section plane = inner wall face (Y=1.3m)
Walls Y min = outer wall face (Y=1.143m)
Wall thickness = 0.157m

Solution: Offset camera back by wall thickness
"""
...
def apply_wall_offset_and_clip(manifest):
    """Offset camera by wall thickness and set clip_start.
    
    Wall thickness = section_plane_y - walls_outer_y
    Camera offset: move back in -Y direction by thickness
    """
    ...
    # Get wall geometry (hardcoded for now, will be in manifest)
    # Section plane Y = 1.3m (inner wall face)
    # Walls Y min = 1.143m (outer wall face)
    section_plane_y = 1.3
    walls_outer_y = 1.143
    wall_thickness = section_plane_y - walls_outer_y
    
    print("=== WALL GEOMETRY ===")
    print(f"Section plane Y (inner): {section_plane_y}m")
    print(f"Walls outer Y: {walls_outer_y}m")
    ...
    # Camera looks +Y, so move back in -Y direction
    camera.location.y -= wall_thickness
    adjusted_y = camera.location.y
    
    print("\n=== CAMERA OFFSET ===")
    print(f"Original Y: {original_y:.3f}m")
    print(f"Offset: -{wall_thickness:.3f}m")
    print(f"Adjusted Y: {adjusted_y:.3f}m")
    
    # Calculate clip_start from adjusted camera to section plane
    clip_start = abs(adjusted_y - section_plane_y)
    camera.data.clip_start = clip_start
    
    print("\n=== CLIP START ===")
    print(f"clip_start = |{adjusted_y:.3f} - {section_plane_y}| = {clip_start:.3f}m")
    
    bpy.context.view_layer.update()
    
    return {
        'section_plane_y': section_plane_y,
        'walls_outer_y': walls_outer_y,
...
def main():
    ...
    print("=" * 60)
    print("B34 — Camera offset by wall thickness")
    print("=" * 60)
    ...
    print("")
    offset_info = apply_wall_offset_and_clip(manifest)
    ...
    experiment = {
        "experiment": "B34",
        "method": "Camera offset by wall thickness (productized)",
        "bundle": str(bundle_dir.name),
        "camera": {
            "fov_horizontal": camera_info['fov_horizontal']
        },
        "wall_geometry": offset_info,
        "formula": "camera_y -= wall_thickness; clip_start = |camera_y - section_plane_y|"
    }
