...
**Промпт для Phase 1 (выбор объектов):**

```
Проанализируй scene_graph.json и beauty.png.

Задача: сопоставить PIDs из scene_graph с объектами из ТЗ.

ТЗ содержит следующие объекты:
1. walls (стены) — surface
2. floor (пол) — surface
3. bathtub (ванна) — fixture
4. vanity (тумба) — fixture
5. shower (душ) — fixture
6. rainshower (лейка) — fixture
7. towel_warmer (полотенцесушитель) — fixture
8. window (окно) — fixture
9. basket (корзина) — fixture
10. mirror (зеркало) — fixture

Для каждого объекта из ТЗ:
1. Найди соответствующий PID в scene_graph по:
   - Имени компонента (Bathtub_classic → bathtub)
   - Размерам и позиции (floor — самый нижний, walls — самый большой)
   - Вложенности (faucet может быть внутри vanity)

2. Определи confidence (0.0-1.0):
   - 1.0 — имя точно совпадает
   - 0.9-0.95 — размеры/позиция однозначны
   - 0.7-0.9 — требуется визуальный анализ

3. Исключи служебные объекты (Sumele — человеческая фигура для масштаба)

Выход: role_map.json в указанном формате.
```
...
| Признак | Действие |
|---------|----------|
| Имя содержит "Sumele", "Person", "Human" | EXCLUDE |
| Объект вне frustum камеры | EXCLUDE |
| face_count = 0 | EXCLUDE (пустой контейнер) |
...
**Промпт для Phase 3 (контроль масок):**

```
Проведи визуальный контроль масок.

Для каждой маски из masks/*.png:

1. Сравни с beauty.png:
   - Покрывает ли маска весь объект? (Coverage)
   - Залезает ли маска на соседние объекты? (Precision)
   - Совпадают ли контуры? (Alignment)

2. Проверь качество маски:
   - Чистый белый (#FFFFFF) на объекте?
   - Чистый чёрный (#000000) вне объекта?
   - Нет ли градиентов, серых зон, артефактов?

3. Поставь оценку 0-100 по каждой метрике.

4. Выяви проблемы:
   - "hollow" — маска только по контуру, пусто внутри
   - "leak" — маска захватывает соседний объект
   - "gray" — серые тона вместо чёрного/белого
   - "misaligned" — контуры не совпадают с beauty

Выход: Scoring Matrix в табличном формате.
```
...
|----------|---------|---------|
| hollow mask | Только edges покрашены | Покрасить все faces рекурсивно |
| object leak | Overlapping geometry | Скрыть соседние объекты при рендере |
| gray tones | Антиалиасинг/материалы | Использовать flat shading |
| misalignment | Разная камера/разрешение | Использовать одну Scene |
